# go-boot-email 项目开发规范文档

go-boot-email 是一个基于 [github.com/xudefa/go-boot](https://github.com/xudefa/go-boot) 的 SMTP 邮件发送集成模块。本模块将 gomail 无缝集成到 go-boot 的 IoC 容器和自动配置体系中，遵循 go-boot 项目的开发规范。
...
- **集成模块**：go-boot-email 是 go-boot 的邮件服务集成，将 SMTP 邮件发送作为通知能力
- **规范继承**：完全遵循 go-boot 的开发规范、命名约定、代码风格

### 1.2 核心职责

- 将 EmailClient 注册为 go-boot 容器中的 Bean
- 提供基于 go-boot 自动配置的邮件服务启动器
- 实现基于 gomail.v2 的 SMTP 邮件发送
- 支持 HTML 格式邮件内容
- 支持 TLS/SSL 加密连接
...
- **邮件库**：集成 gomail（`gopkg.in/gomail.v2`）
- **职责边界**：仅负责邮件发送集成，不包含其他业务逻辑
- **示例代码**：统一放在 `examples/` 目录，演示邮件集成用法

### 2.2 go-boot-email 核心包结构

| 包 | 说明 | 接口定义 |
|---|------|----------|
| `email.go` | EmailClient 实现 | `EmailClient` |
| `autoconfig.go` | 自动配置注册 | `EmailAutoConfiguration` |

### 2.3 go-boot 核心包参考

go-boot-email 依赖 go-boot 的以下核心包：
...
go-boot-email 遵循 go-boot 的接口抽象原则，所有集成层通过核心框架中的接口抽象定义，实现运行时互换：
